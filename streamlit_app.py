...
class ReportSaver:
    ...
    def save_search_results(self, partners: List[PartnerInfo], search_term: str, search_type: str = 'name') -> str:
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"search_{search_type}_{search_term}_{timestamp}.txt"
    ...
    def _load_data(self) -> None:
        ...
        if os.path.exists(self.main_file):
            ...
            try:
                ...
                try:
                    self.filtered_projects_df = pd.read_excel(self.main_file, sheet_name='Projects_from_SI')
                except:
                    pass
                
                try:
                    self.foreign_partners_df = pd.read_excel(self.main_file, sheet_name='Foreign_Partners_SI')
                except:
                    pass
        ...
        if st.session_state.search_results:
            ...
            if st.button("💾 Save Search Results"):
                filepath = st.session_state.report_saver.save_search_results(
                    ...
                    partner_name if partner_name else country_code,
                    'name' if partner_name else 'country'
                )
